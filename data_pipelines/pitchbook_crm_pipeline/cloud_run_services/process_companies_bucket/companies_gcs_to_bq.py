...
SOURCE_BUCKET  = os.environ.get("SOURCE_BUCKET",  "pitchbook-companies")
ARCHIVE_BUCKET = os.environ.get("ARCHIVE_BUCKET", "pitchbook-companies-archive")

TARGET_DATASET = os.environ.get("TARGET_DATASET", "Pitchbook")
TARGET_TABLE   = os.environ.get("TARGET_TABLE",   "companies_raw")
...
# ========= Website HYPERLINK formula extraction =========
def extract_website_urls(xlsx_path: str, df: pd.DataFrame) -> pd.DataFrame:
    """
    Extract URLs from the Website column.
    PitchBook stores these as =HYPERLINK("url","text") formulas which
    pandas reads as the display text rather than the URL.
    """
    if "Website" in df.columns:
        ...
        log("No Website column found — skipping extraction.")
        return df

    wb = load_workbook(xlsx_path, data_only=False)
    ws = wb.active

    website_col_idx = list(df.columns).index(website_col_name) + 1  # 1-based for openpyxl
    href_re = re.compile(r'=HYPERLINK\("([^"]+)"')

    for df_row_idx in range(len(df)):
        excel_row = DATA_START_EXCEL_ROW + df_row_idx
        cell = ws.cell(row=excel_row, column=website_col_idx)
        val = cell.value

        if isinstance(val, str) and val.startswith("=HYPERLINK("):
            m = href_re.search(val)
            if m:
                df.at[df_row_idx, website_col_name] = m.group(1)

    log("Website URL extraction complete.")
    return df
    ...
    try:
        ...
        for i, blob in enumerate(blobs):
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                df = extract_website_urls(local_xlsx, df)
