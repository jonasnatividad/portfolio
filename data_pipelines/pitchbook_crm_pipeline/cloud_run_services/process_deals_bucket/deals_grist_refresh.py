...
PROD_TABLE_FQN = os.environ.get("PROD_TABLE",    f"{PROJECT_ID}.Prod.deals_prod")
...
OVERLAY_TABLE   = os.environ.get("OVERLAY_TABLE",   "overlay_deals_current")
...
GRIST_TABLE   = os.environ.get("GRIST_TABLE",   "Deals"))
...
STATE_BLOB     = os.environ.get("STATE_BLOB",     "states/.state_deals.json")
...
def load_overlay_temp(rows: List[Dict], dataset: str) -> str:
    ...
    temp_fqn = f"{PROJECT_ID}.{dataset}._overlay_deals_upserts_{ts}"
    schema = [SchemaField("deal_id", "STRING"), SchemaField("custom", "JSON")]
...
def merge_overlay(temp_fqn: str, dest_fqn: str) -> None:
    ...
    WHEN NOT MATCHED THEN INSERT (deal_id, custom, updated_at) VALUES (S.deal_id, S.custom, S.ts)
    """
