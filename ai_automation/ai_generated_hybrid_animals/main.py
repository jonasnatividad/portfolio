...
# OpenAI client (prompt + caption generation)
openai_client = OpenAI(api_key=os.environ.get("OPEN_AI_API_KEY"))

# Gemini client (image generation via Nano Banana)
...
prompt = f"Generate a creative and detailed prompt for an AI image generation model to create an imaginative and visually striking image of a hybrid between a {selected_animal_1} and a {selected_animal_2} in a {selected_environment} setting, performing an action of {selected_action}. The final image should only include one creature that is a displays hybrid features of each animal. Include a concise caption suitable for a Twitter post, totaling equal to or less than 185 characters total. This caption should simply describe the hybrid creature and its environment. Format the response as: Image prompt: [prompt here] Caption: [caption here]."
...
# Parse response to separate image prompt and caption
parts = response_content.split("Caption:")
image_prompt = parts[0].replace("Image prompt:", "").strip()
...
twitter_client.create_tweet(text=tweet_content, media_ids=[media_id])
